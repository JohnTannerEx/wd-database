// .vitepress/plugins/icon-replacer.ts

import type { Plugin } from 'vite';

// --- Configuration ---
// The map now connects the link text (e.g., "GitHub") to the full SVG string.
const iconLinkMap: Record<string, string> = {
    ...
    'Source Code': `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-file-binary" viewBox="0 0 16 16"><path d="M5.526 13.09c.976 0 1.524-.79 1.524-2.205 0-1.412-.548-2.203-1.524-2.203-.978 0-1.526.79-1.526 2.203 0 1.415.548 2.206 1.526 2.206zm-.832-2.205c0-1.05.29-1.612.832-1.612.358 0 .607.247.733.721L4.7 11.137a7 7 0 0 1-.006-.252m.832 1.614c-.36 0-.606-.246-.732-.718l1.556-1.145q.005.12.005.249c0 1.052-.29 1.614-.829 1.614m5.329.501v-.595H9.73V8.772h-.69l-1.19.786v.688L8.986 9.5h.05v2.906h-1.18V13h3z"/><path d="M4 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2zm0 1h8a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1"/></svg>`,
...
export function iconReplacerPlugin(): Plugin {
  return {
    ...
    transform(code, id) {
      ...
      const modifiedCode = code.replace(markdownLinkRegex, (match, linkText, url) => {
        // Check if the link text exists in our map.
        if (iconLinkMap[linkText]) {
          const svgIcon = iconLinkMap[linkText];
          // If it exists, create the link with the SVG icon inside.
          return `<a href="${url}" target="_blank" rel="noopener noreferrer" class="icon-link" aria-label="${linkText}">${svgIcon}</a>`;
        }
        
        // If not found, return the original link unchanged.
        return match;
      });
